...
- [x] **I4 — `maxDuration` e runtime Python pinado**: `maxDuration: 60` (limite do Hobby; `ia.processar` roda dentro do request) no `vercel.json` e `.python-version` com `3.12` (a forma suportada de fixar a versão — o campo `runtime` do `builds` não existe para `@vercel/python` e quebrava o build).
